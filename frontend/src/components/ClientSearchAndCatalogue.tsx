...
export default function ClientSearchAndCatalogue({ initialResources }: { initialResources: any[] }) {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full max-w-xl relative flex items-center shadow-xl rounded-2xl bg-white border border-zinc-200 focus-within:ring-4 focus-within:ring-blue-100 transition-all duration-300 hover:scale-[1.02] hover:shadow-blue-500/20 z-10">
      ...
      {query && (
        <div className="mt-6 flex items-center gap-2 text-zinc-500 z-10">
          Showing results for: <span className="font-semibold text-zinc-900">"{query}"</span>
          <button onClick={() => setQuery("")} className="ml-2 text-blue-600 hover:underline text-sm font-medium">Clear search</button>
        </div>
      )}

      {/* Live Resources Catalogue Section */}
      <section className="bg-zinc-50 py-20 px-6 border-t border-zinc-100 w-full mt-12">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-10">
            <Library className="w-8 h-8 text-blue-600" />
            <h2 className="text-3xl font-extrabold text-zinc-900">Recent Study Materials</h2>
          </div>

          {filteredResources.length === 0 ? (
            <div className="bg-white border border-zinc-200 rounded-3xl p-16 text-center max-w-3xl mx-auto shadow-sm">
              <div className="bg-zinc-100 p-5 rounded-full inline-block mb-6">
              ...
              <Link href="/upload">
                <Button size="lg" className="rounded-full bg-blue-600 hover:bg-blue-700 px-8">Be the first to Upload</Button>
              </Link>
            </div>
          ) : (
